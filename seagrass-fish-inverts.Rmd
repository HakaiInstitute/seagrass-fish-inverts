...
```{r}
fish$datecheck <- as.Date(fish$collected_start, "%m/%d/%Y %H:%M") %>% as.character()
fishdatecheck <- fish %>% filter(date != datecheck)
fish$date <- fish$datecheck # 2019-2022 confirmed that collected_start/end contain correct dates, still need to check other dates
fish <- subset(fish, select = -datecheck)
remove(fishdatecheck)

inverts$datecheck <- as.Date(inverts$collected_start, "%m/%d/%Y %H:%M") %>% as.character()
invertsdatecheck <- inverts %>% filter(date != datecheck)
inverts$date <- inverts$datecheck # 2019-2022 confirmed that collected_start/end contain correct dates, still need to check other dates
...
Everything from 2019-2022 passes event-level QC; add earlier years later once rest of QC is sorted
```{r}
