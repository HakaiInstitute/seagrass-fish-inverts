...
# add corresponding acceptable fish names to replace ones that need fixing
invertsnames_tofix$replace <- c("") 

inverts <- FindReplace(inverts, "species", invertsnames_tofix, from = "name", to = "replace", exact = TRUE, vector = FALSE)
```

We seem to have the same naming issue for some other variables; let's fix this for the "collector" (i.e. diver) variable since that comes up later
```{r}
...
# create list of fish names in data that need fixing
diversnames_data <- unique(fish$collector) %>% sort %>% data.frame
colnames(diversnames_data) <- "name"
diversnames_tofix <- anti_join(diversnames_data, diversnames_accepted)

# add corresponding acceptable diver names to replace ones that need fixing
diversnames_tofix$replace <- c("andrew", "derek", "derek", "kyle", "ondine", "other", "zach") 

fish <- FindReplace(fish, "collector", diversnames_tofix, from = "name", to = "replace", exact = TRUE, vector = FALSE)
```
