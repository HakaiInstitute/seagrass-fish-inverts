...
Remove variables that are for portal use only
```{r}
events <- subset(events, select = -c(action, samples))
fish <- subset(fish, select = -action)
inverts <- subset(inverts, select = -action)
```
...
### Portal issues fix (these may be removed once the issues are resolved)

Portal issues caused some rows to be added instead of updated (Hakai IDs should not appear more than once), so remove duplicate/extra rows
```{r}
...
Collected_start/end times for 2016 are appearing as 2012, so correct that
```{r}
...
Fix diver names (i.e. collectors) in both data sets so there aren't multiple names for the same person
```{r}
# replacement using FindReplace
...
Check for discrepancies between date and collected_start/end (collected_start/end typically contain the correct date, but important to check for entering mistakes):
```{r}
fish$datecheck <- as.Date(fish$collected_start) %>% as.character()
fishdatecheck <- fish %>% filter(date != datecheck)
fish$date <- fish$datecheck # 2019-2021 confirmed that collected_start/end contain correct dates, still need to check other dates
fish <- subset(fish, select = -datecheck)
remove(fishdatecheck)

inverts$datecheck <- as.Date(inverts$collected_start) %>% as.character()
invertsdatecheck <- inverts %>% filter(date != datecheck)
inverts$date <- inverts$datecheck # 2019-2021 confirmed that collected_start/end contain correct dates, still need to check other dates
...
In order to ensure completeness of the fish and invertebrate data sets (i.e. all
...
### Miscellaneous fixes re: event issues to QC

New rows added to 2021 inverts data set on portal via excel file missing collector name; fix that here
```{r}
inverts$collector <- replace(inverts$collector, grepl("MGINV48083", inverts$hakai_id), "derek")
inverts$collector <- replace(inverts$collector, grepl("MGINV48084", inverts$hakai_id), "zach")
inverts$collector <- replace(inverts$collector, grepl("MGINV48085", inverts$hakai_id), "derek")
```
...
```{r}
# note that for abundance, we'll want to group quantity by species and transects (i.e. unique combos of site_id, date, and collector) to look at average quantity per transect
