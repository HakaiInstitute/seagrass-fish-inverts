...
### Portal issues fix (these may be removable once the issues are resolved)
...
Collection start and end times for 2016 are appearing as 2012, so correct that
```{r}
fish$collected_start <- gsub("2012", "2016", fish$collected_start)
fish$collected_end <- gsub("2012", "2016", fish$collected_end)
```

Note: may need to enter QC step to check that date, collected_start, and collected_end all match; date is easily misentered when forms are set up ahead of time, so collected_start/end typically contain the correct date
...
Convert all blank cells to NA and remove NA columns
```{r}
